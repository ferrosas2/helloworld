...
![Architecture Diagram](./skill_1_1_1_full.png)
...
4.  Saves outputs (Markdown summary & JSON result) to a local `outputs/` folder.
5.  Uploads the generated files to S3 under the `bonus_1.1/outputs/` prefix.
...
3.  Results are aggregated and saved to `outputs/model_comparison_results.json`.
4.  All local outputs are automatically synced to the S3 bucket.
5.  Performance metrics (Total Time, Extraction Time, Summary Time) are calculated and compared.
